...
const OrderConfigPage = () => {
    ...
    useEffect(() => {
        ...
        setTimeout(() =>
            getCarConfiguration().catch(() => setErrored(true)), 1000)
    }, [modelId]);
    ...
    return (
        <div className="order-config-main">
            ...
            <div className="order-config-content" style={{ flexDirection: desktopMode ? 'row' : 'column' }}>
                ...
                {errored && <div className="order-config-error"><ErrorState /></div>}

                {!errored && carConfig == undefined && <div className="order-config-error"><LoadingState /></div>}

                {!errored && carConfig != undefined &&
